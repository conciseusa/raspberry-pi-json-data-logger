...
echo "$DATE - CMDF = $CMDF"
echo "$DATE - $SCRIPT_DIR - $0" >> $LOG_DIR/remote-cmd.log # for troubleshooting in cron jobs
echo "$DATE - CMDF = $CMDF" >> $LOG_DIR/remote-cmd.log
...
echo "$DATE - pull = $pull"
if [ $pull = "T" ]; then
   echo "$DATE - pull" >> $LOG_DIR/remote-cmd.log
   git -C $SCRIPT_DIR pull >> $LOG_DIR/remote-cmd.log
fi
...
if [ $reboot = "T" ]; then
   echo "$DATE - reboot" >> $LOG_DIR/remote-cmd.log
   /sbin/reboot
fi
...
if [ 1 = 1 ]; then # 0 = 1 disable until needed, 1 = 1 enable
   data=`curl -s $CMDF | python -c 'import json,sys;obj=json.load(sys.stdin);print(obj["'data'"])'`;
   echo "$DATE - data = $data"
   echo "$DATE - data = $data" >> $LOG_DIR/remote-cmd.log
   echo "$data" > $LOG_DIR/remote-cmd.txt # set to file to update
fi
